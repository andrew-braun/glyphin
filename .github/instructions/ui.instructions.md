...
- Use `@lucide/svelte` for icons. Import each icon from `@lucide/svelte/icons/<icon-name>`; do not use the package root barrel or text characters such as `->` to stand in for icons. For forward-action button labels, prefer `ButtonForwardLabel.svelte`.
