# Georgian Reading DB Ingestion Strategy

This file plans how reviewed authoring artifacts for `georgian-reading-v1` should move
...
- Should Georgian consonant contrast families be stored on grapheme metadata or
  as orthography and pronunciation rules linked to drill examples?
- Should Mtavruli sign variants be modeled now or deferred until a reviewed
  target-domain sample requires them?
- Should long repeated chunks in `შესასვლელი` and `გასასვლელი` be stored as
  support-word segments even before all component letters are introduced?
