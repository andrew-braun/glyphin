# Brahmi Reading Review Packet

Generated from `brahmi-reading-v1` authoring artifacts.

## Review Gates

- [ ] Segmentation and grapheme mapping
- [ ] Pronunciation or romanization
- [ ] Cultural context and learner-facing wording
- [ ] License and attribution
- [ ] Database ingestion readiness

## Course Summary

- Course ID: `brahmi-reading-v1`
- Language tag: `sa-Brah`
- Script: `Brah`
- Direction: `ltr`
- Target domains: ashokan_edict_terms, museum_inscriptions, donative_labels, royal_names_titles, transliteration_workbenches

## Sources To Review

| Source                        | Kind                 | Use             | License                                       | Notes                                                                                 |
| ----------------------------- | -------------------- | --------------- | --------------------------------------------- | ------------------------------------------------------------------------------------- |
| unicode-brahmi-block          | unicode_metadata     | scoring         | Unicode License reviewed for metadata use     | Encoded Brahmi characters and combining marks for inventory and rendering checks.     |
| siddham-inscription-database  | epigraphic_database  | discovery_only  | database and image reuse terms require review | South Asian inscription context and bibliography for discovery only.                  |
| gretel-sanskrit-reference     | text_reference       | discovery_only  | text-specific reuse terms require review      | Sanskrit lexical context only and not an inscription corpus for shipped examples.     |
| ashoka-inscription-references | epigraphic_reference | discovery_only  | reuse terms require review                    | Edict terminology and bibliography for human research only.                           |
| noto-sans-brahmi              | font_reference       | scoring         | SIL Open Font License                         | Rendering reference for Brahmi letters and vowel signs.                               |
| app-authored-brahmi-examples  | authored_examples    | shipped_content | app_owned                                     | Short reviewed Brahmi examples authored by Glyphin from cleared vocabulary decisions. |

## Candidate Highlights

| Type   | Candidate | Gloss          | Score  | Notes                                                     |
| ------ | --------- | -------------- | ------ | --------------------------------------------------------- |
| anchor | 𑀤𑀸𑀦       | dana gift      | 0.5228 | Donative label anchor with aa sign payoff                 |
| anchor | 𑀫𑀳𑀸       | maha great     | 0.4988 | Common modifier and aa sign review                        |
| anchor | 𑀭𑀸𑀚       | raja king      | 0.4676 | High royal and inscriptional payoff                       |
| anchor | 𑀦𑀫𑁄       | namo homage    | 0.3844 | Liturgical or inscriptional term with context needs       |
| anchor | 𑀅𑀰𑁄𑀓      | Asoka          | 0.3092 | Royal name anchor with extra vowel-sign load              |
| anchor | 𑀯𑀺𑀚𑀬      | vijaya victory | 0.3072 | Name and royal-domain word with i sign                    |
| anchor | 𑀥𑀭𑁆𑀫      | dharma         | 0.2676 | High cultural payoff but virama and concept load are high |
| anchor | 𑀅𑀬𑀁       | ayam this      | 0.2384 | Short grammar-heavy candidate for later review            |
| anchor | 𑀮𑀸        | laa sequence   | 0.2208 | Low-confidence vowel-sign practice sequence               |
| anchor | 𑀲𑀯        | sa va sequence | 0.2032 | Reviewer-gated practice sequence                          |
| anchor | 𑀲𑀁𑀖       | sangha         | 0.1664 | Cultural term with anusvara and aspiration review         |
| anchor | 𑀩𑁆𑀭𑀸𑀳𑁆𑀫𑀻  | Brahmi         | 0.0668 | Script-name anchor is late because conjunct load is high  |

## Reviewer Notes

### Segmentation

-

### Pronunciation

-

### Cultural Context

-

### License And Attribution

-

### DB Ingestion

-
