# Cherokee Reading DB Ingestion Strategy

This file plans how reviewed authoring artifacts for `cherokee-reading-v1` should
...
| Reviewed syllabograms      | `curriculum.graphemes`, `curriculum.course_version_graphemes`                                     |
| Case and series metadata   | `curriculum.course_version_graphemes.metadata` pending schema convention                          |
...
- Store Cherokee syllabograms as graphemes with script-specific series metadata.
...
- Activate the publication only after smoke checks confirm payload shape,
  Cherokee font rendering, and lesson ordering.
...
- [ ] Every syllabogram maps to one script system and one course-version pedagogy
      row.
...
- Should Cherokee lowercase be ingested in v1 or deferred until case-aware UI is
  tested?
...
- Should syllabary series be a generic metadata convention or a Cherokee-specific
  field in the publication payload?
