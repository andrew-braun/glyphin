# Syriac Reading DB Ingestion Strategy

This file plans how reviewed authoring artifacts for `syriac-reading-v1` should
...
- Store Syriac strings in NFC with explicit `direction = rtl` and script style
  metadata.
- Store contextual joining as rule or grapheme metadata rather than duplicating
  presentation forms.
- Build lesson-level runtime payloads from normalized rows, not from ad hoc UI
  data.
- Activate a `delivery` publication only after bidi, font shaping, and payload
  smoke checks pass.
...
- [ ] Every combining mark or optional vowel point has an explicit storage and
      display decision.
...
- Should optional vowel points become separate `curriculum.graphemes` rows or
  remain lesson-level annotations?
- Which reviewed fields are allowed to become learner-visible pronunciation copy?
- Which font family is approved for production Syriac rendering?
