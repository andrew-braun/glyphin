# Vai Reading DB Ingestion Strategy

This file plans how reviewed authoring artifacts for `vai-reading-v1` should move
...
| Reviewed Vai syllabograms             | `curriculum.graphemes`, `curriculum.course_version_graphemes`                                     |
| Series and source-confidence metadata | `curriculum.course_version_graphemes.metadata` pending convention                                 |
...
- Activate the publication only after smoke checks confirm payload shape, font
  support, source confidence, and lesson ordering.
...
- [ ] Every Vai syllabogram maps to one script system and one course-version
      pedagogy row.
...
- Should Vai series grouping become a generic syllabary metadata convention?
