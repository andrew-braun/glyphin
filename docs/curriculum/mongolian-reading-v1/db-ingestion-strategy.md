# Mongolian Reading DB Ingestion Strategy

This file plans how reviewed authoring artifacts for `mongolian-reading-v1`
...
- Store Mongolian strings in NFC while preserving variation sequences and audit
  copies.
- Store `direction = ttb` and a future writing-mode value so runtime surfaces can
  choose vertical layout.
- Build lesson-level runtime payloads from normalized rows, not from ad hoc UI
  data.
- Activate a `delivery` publication only after vertical layout, font shaping, and
...
- [ ] Variation selectors and shaping controls have explicit storage decisions.
...
- Should database metadata distinguish direction from CSS writing mode?
- Should variation sequences be stored as grapheme rows, variant metadata, or raw
  text features with reviewer notes?
