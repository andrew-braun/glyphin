# Malayalam Reading DB Ingestion Strategy

This file plans how reviewed authoring artifacts for `malayalam-reading-v1`
...
- Should Malayalam chillus be stored as distinct graphemes, base-consonant
  variants, or rule-linked segment rows?
- How should chandrakkala and conjunct sequences be represented so `ക`, `കാ`,
  `ക്കി`, and `ൽ` can be scored as both visible clusters and teachable parts?
- Does the delivery payload need a script-shaping metadata block for font family,
  cluster boundaries, chillu forms, conjunct forms, and mark-placement hints?
