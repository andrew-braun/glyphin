# Tifinagh Reading DB Ingestion Strategy

This file plans how reviewed authoring artifacts for `tifinagh-reading-v1`
...
- Store Tifinagh strings in NFC with `direction = ltr` and script-specific font
  metadata.
- Store morphology chunks as rule examples until a reusable chunk model exists.
- Build lesson-level runtime payloads from normalized rows, not from ad hoc UI
  data.
- Activate a `delivery` publication only after font and payload smoke checks
  pass.
...
- [ ] Every grapheme has a reviewer-approved romanization or pronunciation hint.
...
- Which transliteration or pronunciation style should be stored for learner
  display?
- Should morphology chunks be stored as grapheme-like units or orthography rules?
