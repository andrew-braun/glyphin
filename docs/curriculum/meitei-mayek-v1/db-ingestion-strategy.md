# Meitei Mayek Reading DB Ingestion Strategy

This file plans how reviewed authoring artifacts for `meitei-mayek-v1` should
...
- Store Meitei Mayek strings in NFC and preserve final-letter code points exactly
  as reviewed.
- Store lonsum final-letter behavior as reviewed grapheme details or rule
  metadata before publication.
- Build lesson-level runtime payloads from normalized rows, not from ad hoc UI
  data.
- Activate a `delivery` publication only after font rendering and payload smoke
  checks pass.
...
- [ ] Every vowel sign and lonsum final letter has an explicit segmentation
      decision.
...
- Should lonsum final letters be stored as independent graphemes or final-position
  metadata?
- Which font family is approved for production Meitei Mayek rendering?
