# Cree Syllabics Reading DB Ingestion Strategy

This file plans how reviewed authoring artifacts for `cree-syllabics-v1` should
...
| Reviewed syllabic signs and finals | `curriculum.graphemes`, `curriculum.course_version_graphemes`                                     |
| Orientation and dialect metadata   | `curriculum.course_version_graphemes.metadata` pending schema convention                          |
...
- Store syllabic signs and finals with role metadata so drills can distinguish
  syllables from finals.
...
- Activate the publication only after smoke checks confirm payload shape,
  syllabics font rendering, and lesson ordering.
...
- [ ] Every syllabic sign maps to one script system and one course-version
      pedagogy row.
...
- Which dialect and orthography profile should become `course_versions` metadata?
...
- Should finals and long-vowel marks use generic grapheme roles or a dedicated
  syllabics metadata convention?
