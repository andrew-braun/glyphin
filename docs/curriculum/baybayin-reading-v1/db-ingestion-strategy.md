# Baybayin Reading DB Ingestion Strategy

This file plans how reviewed authoring artifacts for `baybayin-reading-v1`
...
- Store Baybayin strings in NFC and preserve final-consonant notation exactly as
  reviewed.
- Store the selected historic or modern orthography policy as course-version
  metadata before publication.
- Build lesson-level runtime payloads from normalized rows, not from ad hoc UI
  data.
- Activate a `delivery` publication only after font rendering, cultural review,
  and payload smoke checks pass.
...
- [ ] Final-consonant policy is approved and consistently applied.
...
- Should v1 store a modern virama convention, a historic open-syllable convention,
  or both as separate course modes?
- Which font family is approved for production Baybayin rendering?
- Which reviewed fields are allowed to become learner-visible cultural notes?
