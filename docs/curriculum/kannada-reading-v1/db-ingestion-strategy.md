# Kannada Reading DB Ingestion Strategy

This file plans how reviewed authoring artifacts for `kannada-reading-v1` should
...
- Should Kannada virama and ottakshara forms be stored as distinct graphemes,
  base-consonant variants, or rule-linked segment rows?
- How should dependent vowel signs be represented so `ಕ`, `ಕಾ`, `ಕಿ`, and `ಕೈ`
  can be scored as both visible clusters and teachable parts?
- Does the delivery payload need a script-shaping metadata block for font family,
  cluster boundaries, subjoined forms, and mark-placement hints?
