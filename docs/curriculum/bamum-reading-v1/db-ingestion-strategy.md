# Bamum Reading DB Ingestion Strategy

This file plans how reviewed authoring artifacts for `bamum-reading-v1` should
...
| Reviewed Bamum signs       | `curriculum.graphemes`, `curriculum.course_version_graphemes`                                     |
| Script-stage metadata      | `curriculum.course_version_graphemes.metadata` pending convention                                 |
...
- Replace placeholder anchors with reviewer-approved vocabulary before any
  `delivery.*` publication is created.
- Store script-stage metadata before adding any Bamum Supplement signs.
...
- Activate the publication only after smoke checks confirm payload shape, font
  support, script-stage boundaries, and lesson ordering.
...
- [ ] Every Bamum sign maps to one script system and one course-version pedagogy
      row.
...
- Should placeholder anchors be ingested as draft curriculum rows or blocked
  until replaced?
...
- Should script-stage metadata be generic across Bamum, Egyptian, Linear B, and
