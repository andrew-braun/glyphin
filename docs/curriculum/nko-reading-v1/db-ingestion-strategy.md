# N'Ko Reading DB Ingestion Strategy

This file plans how reviewed authoring artifacts for `nko-reading-v1` should move
...
| Reviewed N'Ko letters and marks | `curriculum.graphemes`, `curriculum.course_version_graphemes`                                        |
| Tone and bidi metadata          | `curriculum.course_version_graphemes.metadata` and `curriculum.courses.ui_config` pending convention |
...
- Store combining marks as reviewable pedagogical units with base-letter examples.
...
- Activate the publication only after smoke checks confirm payload shape, RTL
  rendering, mark positioning, font support, and lesson ordering.
...
- [ ] Every N'Ko letter or mark maps to one script system and one course-version
      pedagogy row.
...
- Should combining marks be first-class graphemes or metadata attached to base
...
- How should N'Ko tone and transliteration appear in accessible labels?
